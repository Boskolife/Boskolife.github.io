...
if (burger) {
    burger.addEventListener("click", () => {
        ...
        if (arrowTabWrap)
            arrowTabWrap.classList.remove("arrow_tab_wrap_active");
...
class ItcTabs {
    constructor(target, config) {
        ...
        if (this._cardElButtons)
            this._navigationElements.push(this._cardElButtons);
        this._init();
        this._events();
    }
    _init() {
...
function openCatalog() {
  ...
  const catalogList = document.querySelector(".sub-menu");
  let isOpen = false;
